...
- `app.js`
  - 서버 어플리케이션의 entry point
  - **static 관련 코드가 추가**될 것입니다.
- `src`
  - `components.js`
    - 앞선 코드와 동일합니다.
  - `render.js` → `ServerRenderer.js`
  - `main.js`
    - **클라이언트 어플리케이션**의 entry point
  - `store.js`
    - 이전과 동일합니다.
...
- 간단한 api를 통해서 server에서 관리해주는 데이터를 변경할 수 있도록 만들어줍니다.
- 그리고 렌더링 시점에 state를 넘겨줄 수 있도록 작업이 필요합니다.
